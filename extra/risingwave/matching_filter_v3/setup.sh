#!/usr/bin/env bash
# Apply the matching_filter v3 shadow on top of a running production pipeline
# (../setup.sh must have been applied: it reads flat_filters_agg_mv,
# flat_filters_agg (fallback), subscriptions_agg, events_enriched and the
# production UDFs).
#
...
#
# Usage: [SHADOW_SINCE=<timestamptz>] ./extra/risingwave/matching_filter_v3/setup.sh
set -euo pipefail
...
echo "==> Done. Compare with: ./extra/risingwave/matching_filter_v3/parity.sh"
