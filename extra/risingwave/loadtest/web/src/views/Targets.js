...
export function Targets({ discovery, selected, probeTargetId, walletProbeTargetId, seedSpec, setSeedSpec, maxVariantsPerTarget, onDiscovered, onSelect, onProbe, onWalletProbe, }) {
    ...
    const visibleSubs = useMemo(() => [...bySub.entries()].filter(([sub, list]) => {
        if (!q)
            return true;
        const first = list[0];
        return (sub.toLowerCase().includes(q) ||
        ...
        : walletFeeds.length === 0 && !walletProbeFeeds
            ...
            : walletAligned && !shared
                ...
                : walletPriceable
                    ? "watermark"
                    : "refresh";
    return (_jsxs(_Fragment, { children: [err && _jsx(Banner, { kind: "bad", children: err }), _jsx(SeedCard, { spec: seedSpec, setSpec: setSeedSpec, onSeeded: run, maxVariantsPerTarget: maxVariantsPerTarget }), _jsxs(Card, { title: "Discover targets", hint: "reads subscriptions \u2192 plan \u2192 charges \u2192 billable metrics from the Lago API", right: _jsx("button", { className: "btn primary", onClick: run, disabled: busy, children: busy ? "Scanning…" : discovery ? "Rescan" : "Scan Lago" }), children: [!discovery && _jsx("p", { style: { color: "var(--text-secondary)" }, children: "Scan to list what this instance can be load-tested against." }), discovery && (_jsxs("p", { style: { color: "var(--text-secondary)", fontSize: 13 }, children: [discovery.subscriptions.length, " active subscription(s), ", discovery.targets.length, " chargeable metric target(s), scanned ", new Date(discovery.scannedAt).toLocaleTimeString(), ". Only active subscriptions whose plan actually charges for a metric are offered \u2014 an event for an uncharged metric never reaches the expanded stage, so its latency could not be measured past stage 0."] })), discovery?.warnings.map((w) => (_jsx("div", { style: { marginTop: 8 }, children: _jsx(Banner, { kind: "warn", children: w }) }, w)))] }), discovery && discovery.targets.length > 0 && (_jsxs(Card, { title: "Bulk load targets", hint: `${num(selected.size)} of ${num(discovery.targets.length)} selected across ${num(bySub.size)} subscription(s) — events are spread round-robin across them`, right: _jsxs("div", { className: "row", children: [seeded.length > 0 && (_jsxs(_Fragment, { children: [_jsx("button", { className: "btn", title: `select every target of the ${seeded.length} subscription(s) whose id starts with "${seedSpec?.prefix}"`, onClick: () => selectSeeded(false), children: "Select seeded" }), _jsx("button", { className: "btn", title: "select the seeded subscriptions except the last one, and point the usage probe (and the wallet " +
                                        "probe if it has a wallet) at that spare one — so the probe runs in exact mode under the bulk load", onClick: () => selectSeeded(true), disabled: seeded.length < 2, children: "Select seeded, probe on the spare" })] })), _jsx("button", { className: "btn", onClick: () => onSelect(new Set(discovery.targets.map((t) => t.id))), children: "Select all" }), _jsx("button", { className: "btn", onClick: () => onSelect(new Set()), children: "Clear" })] }), children: [compact && (_jsxs("div", { className: "row", style: { marginBottom: 10 }, children: [_jsx("input", { type: "text", placeholder: "filter by subscription, customer, plan or metric code", value: query, onChange: (e) => setQuery(e.target.value), style: { flex: 1, minWidth: 260 } }), _jsxs("span", { style: { color: "var(--text-muted)", fontSize: 12 }, children: [num(visibleSubs.length), " of ", num(bySub.size), " subscription(s) shown \u00B7 collapsed; expand one to pick a probe"] }), _jsx("button", { className: "btn", onClick: () => setExpanded(new Set()), disabled: expanded.size === 0, children: "Collapse all" })] })), _jsxs("div", { className: "targets", children: [visibleSubs.map(([sub, list]) => {
                                const first = list[0];
