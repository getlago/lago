                    ...
                    : "stored in a local SQLite database next to the app", right: _jsx(HealthPills, { health: health, onRefresh: onCheck }), children: _jsxs("div", { className: "grid cols-3", children: [T({
                            ...
                            note: config.clickhouse.passwordSet ? "leave blank to keep the stored password" : "no password stored yet",
                                ...
                                note: "both consumers read http_ruby as \"custom expressions already evaluated\" — changing it measures a different code path",
                                    ...
                                    })] }))] }), _jsxs("p", { style: { marginTop: 12, fontSize: 12, color: "var(--text-muted)" }, children: ["The message produced is byte-shape identical to what ", _jsx("code", { children: "Events::KafkaProducerService" }), " writes \u2014", _jsx("code", { children: "timestamp" }), " as a JSON string of float seconds, ", _jsx("code", { children: "ingested_at" }), " without the trailing", " ", _jsx("code", { children: "Z" }), ", ", _jsx("code", { children: "precise_total_amount_cents" }), " defaulting to ", _jsx("code", { children: "\"0.0\"" }), ". The partition key is the one deliberate difference: the API keys by", " ", _jsx("code", { children: "<organization_id>-<external_subscription_id>" }), ", but a run has only as many keys as it has targets, which pins all its traffic to that many partitions and caps RisingWave's source parallelism to the same number \u2014 so load runs produce unkeyed by default and round-robin over every partition. Nothing downstream is partition-affine. Set ", _jsx("code", { children: "kafka.partitionKey" }), " to ", _jsx("code", { children: "subscription" }), " to reproduce the API's own partitioning. What it cannot reproduce is what the API does ", _jsx("em", { children: "besides" }), " producing: for an organization whose events store is Postgres, the ", _jsx("code", { children: "events" }), " row and ", _jsx("code", { children: "PostProcessJob" }), " never happen, so any usage read not served by the realtime 15-minute buckets has nothing to read. Preflight says which case this organization is in."] })] }), _jsx(Card, { title: "HTTP to Lago", hint: "the send path's own limits \u2014 throughput is in-flight requests divided by round trip, and these decide what in-flight can be", children: _jsxs("div", { className: "grid cols-4", children: [N({
