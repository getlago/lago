#!/usr/bin/env bash
# Row-level diff of events_expanded (production) vs events_expanded_slim
# (shadow) over the window the shadow has been running.
#
...
#
# Usage: ./extra/risingwave/slim_filters/parity.sh [settle_seconds=30] [sample=20]
set -euo pipefail
...
slim AS (
    SELECT s.* FROM events_expanded_slim s, win
    WHERE s.kafka_timestamp >= win.lo AND s.kafka_timestamp < win.hi
),
diff AS (
    SELECT
        CASE
            WHEN s.transaction_id IS NULL THEN 'missing_in_slim'
            WHEN p.transaction_id IS NULL THEN 'extra_in_slim'
...
SELECT
    ...
    s.charge_filter_id AS slim_filter,
    p.grouped_by AS prod_grouped_by,
    s.grouped_by AS slim_grouped_by
FROM (
    SELECT * FROM events_expanded
    WHERE kafka_timestamp >= (SELECT min(kafka_timestamp) FROM events_expanded_slim)
      AND kafka_timestamp < now() - (:'settle' || ' seconds')::interval
) p
FULL OUTER JOIN (
    SELECT * FROM events_expanded_slim
