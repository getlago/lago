...
-- Source of truth: extra/risingwave/udf/src/matching_filter_v2.rs (+
-- json_text.rs); cargo test checks it against matching_filter (v1) on the v1
-- cases, a 20k-case fuzz and a 2,300-filter charge. Regenerate with
-- extra/risingwave/udf/gen_sql.sh.
--
-- matching_filter_v2(filters_text, properties) -> position of the winning
-- candidate in the charge's filters_agg array (0-based), or -1 for the
-- charge's default bucket. See the source header for why it is cheaper than
-- matching_filter on large filter sets.

CREATE FUNCTION IF NOT EXISTS matching_filter_v2(filters_text VARCHAR, properties JSONB) RETURNS INT
LANGUAGE rust AS $$
