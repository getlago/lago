...
| `sql/02_flat_filters.sql` | Rebuild of the Postgres `flat_filters` view, then the stage-1 lookup ARRAYS: `flat_filters_agg` (one row per charge, candidates as a jsonb array) and `subscriptions_agg` (one row per external_id) — MV → sink-into-table so they can be temporal-joined |
| `sql/03_functions.sql` | GENERATED (by `udf/gen_sql.sh`, do not hand-edit): embedded RUST UDFs `matching_filter`, `pick_subscription`, `extract_grouped_by` — line-by-line ports of the Go processor's `MatchingFilter` / `FetchSubscription` / `enrichWithPricingGroupKeys`, unit-tested in `udf/` |
| `sql/04_enrichment.sql` | Bounded pipeline: sink (BM join → 32d filter → first-wins dedup) INTO append-only TABLE `events_enriched` (retention 33d) → sink (two single-row temporal joins + scalar UDFs, ranking-free since 2026-08-28 — ROADMAP §0c) INTO append-only TABLE `events_expanded` (retention 33d). Both tables carry the clocks (`kafka_timestamp`, `rw_received_at`); `events_expanded` adds `rw_expanded_at DEFAULT now()`, stamped at insert because the sink omits that column — `proctime()` and bare `now()` are both rejected in a streaming projection |
| `udf/` | Cargo crate holding the UDF Rust sources + Go-parity test suite (`cargo test`); `gen_sql.sh` assembles `sql/03_functions.sql` from them verbatim |
